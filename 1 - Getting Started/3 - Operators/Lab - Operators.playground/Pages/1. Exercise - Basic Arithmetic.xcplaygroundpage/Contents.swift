...
// It happens because it considered values to be of int type(without any decimal) but after making it double explicit type it provided output with decimal values
/*:
...
// we can use the remainder operator to determine if an integer is even or odd by dividing the value by 2. So whenever the remainder operator returns 0 then the number is even and whenever the remainder operator returns zero then the number is odd(by basic mathematics 😀)
/*:
